...
const FormCard = ({ n, title, children }) => (
  <div
...
const Checkout = () => {
  ...
  const validate = () => {
    ...
    if (!PHONE_RE.test(form.phone.replace(/\s+/g, ''))) next.phone = 'Enter a valid BD mobile number.'
  ...
  const hint = {
    ...
    bkash: 'You will be sent to bKash to confirm the payment, then returned here with your order number.',
    nagad: 'You will be sent to Nagad to confirm the payment, then returned here with your order number.',
  ...
  return (
    <>
      ...
      <div className="mx-auto flex max-w-[1680px] flex-wrap items-center gap-2 px-[clamp(20px,4vw,64px)] pt-[clamp(24px,3vw,40px)] font-mono text-[10.5px] uppercase tracking-[0.16em]">
        <Link to="/cart" className="text-snow/45 transition-colors hover:text-[#9ac8f5]">
          Cart
        </Link>
        ...
        <aside className="flex min-w-[min(100%,280px)] max-w-[420px] flex-1 basis-80 flex-col gap-3.5 lg:sticky lg:top-24">
          ...
          >
            <div className="mb-5 flex items-center justify-between gap-3">
              ...
              <Link to="/cart" className="font-body text-[12px] font-semibold text-[#7fb0e4]">
                Edit cart
              </Link>
            </div>
            ...
            <div className="mb-4.5 grid gap-3.5 border-b border-white/8 pb-4.5">
              {items.map((line) => {
                const Thumb = line.product.image
                return (
                  <div key={line.id} className="flex items-center gap-3.25">
                    <div
                      className="h-13 w-13 shrink-0 overflow-hidden rounded-xl border border-white/9 bg-surface-media"
                      style={{

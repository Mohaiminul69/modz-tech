...
import { fmt } from "@utils/cartPricing";
...
const useCountdown = (endDate: Date): CountdownUnit[] => {
  ...
  const value = (n: number) => (ms === null ? "--" : pad(n));

  return [
    { label: "Days", value: value(Math.floor((ms ?? 0) / 86400000)) },
    { label: "Hrs", value: value(Math.floor((ms ?? 0) / 3600000) % 24) },
    { label: "Min", value: value(Math.floor((ms ?? 0) / 60000) % 60) },
    { label: "Sec", value: value(Math.floor((ms ?? 0) / 1000) % 60) },
  ];
};

const PartThumb = ({ part }: { part: { name: string; image: ShopProduct["image"] } }) => (
  <div className="min-w-0 flex-1">
    <div className="relative h-[118px] overflow-hidden rounded-[14px] border border-white/9 bg-surface-media">
      {part.image ? (
        <Image src={part.image} alt={part.name} fill sizes="140px" className="object-cover" />
      ) : (
        <div className="flex h-full w-full items-center justify-center">
          <PiCube className="h-8 w-8 text-accent-tint/60" strokeWidth={1.5} />
        </div>
      )}
    </div>
...
const ProductDetail = () => {
  ...
  const description = product.details?.description || [product.note];
  const gallery = product.details?.gallery || fallbackGallery(product);
  const active = gallery?.[view] || gallery?.[0];
  const specs = product.details?.specs || genericSpecs(product);
  const Icon = ICONS[product.icon] || PiCube;

  const relatedProducts = product.details?.related
  ...
  return (
    <>
      ...
      <div className="mx-auto flex max-w-[1680px] flex-wrap items-center gap-2 px-[clamp(20px,4vw,64px)] pt-[clamp(24px,3vw,40px)] font-mono text-[10.5px] uppercase tracking-[0.16em] text-snow/45">
        ...
        <span>/</span>
        <Link href="/shop" className="text-snow/45 transition-colors hover:text-[#9ac8f5]">
          Shop
        </Link>
        <span>/</span>
        <Link href="/shop" className="text-snow/45 transition-colors hover:text-[#9ac8f5]">
          ...
          {gallery && (
            <div className="grid grid-cols-4 gap-2.5">
              {gallery.map((g, i) => (
                ...
                >
                  <span
                    className={`relative block aspect-square w-full overflow-hidden rounded-[14px] border bg-surface-media transition-colors hover:border-[rgba(106,169,233,.5)] ${
                      i === view ? "border-[rgba(106,169,233,.6)]" : "border-white/8"
                    }`}
                  >
                    {g.image ? (
                      <Image src={g.image} alt={g.label} fill sizes="100px" className="object-cover" />
                    ) : (
                      <span className="flex h-full w-full items-center justify-center">
                        <PiCube className="h-5 w-5 text-accent-tint/60" strokeWidth={1.5} />
                      </span>
                    )}
                  </span>
                  <span
        ...
        <div className="mx-auto flex max-w-[1680px] flex-wrap items-start gap-[clamp(24px,3.5vw,56px)]">
          <div className="min-w-[min(100%,260px)] flex-1 basis-75">
            ...
            {description.map((p) => (
              <p
                key={p}
                className="mb-4 max-w-[440px] font-body text-[14.5px] leading-[1.7] text-snow/60 last:mb-0"
              >
                {p}
              </p>
            ))}
      ...
      {/* Bundles */}
      {product.details?.bundles && product.details.bundles.length > 0 && (
        <section
        ...
        >
          <div className="mx-auto max-w-[1680px]">
            ...
            >
              {product.details.bundles.map((bundle) => {
        ...
        <section className="border-t border-white/7 px-[clamp(20px,4vw,64px)] py-[clamp(48px,6vw,96px)]">
          <div className="mx-auto max-w-[1680px]">
            <div className="mb-9 flex flex-wrap items-end justify-between gap-5">
              ...
              >
                {product.details?.related ? "All accessories →" : "See more →"}
              </Link>
            </div>
